...
def solanart_nft(search_address):
    x=1
    current_time=time.time()
    #solanartのtxを抽出
    sol_detail=solana_client.get_signatures_for_address(search_address, limit=1000)
    while True:
        try:
            #tx抽出
            ...
            if block_time < (current_time-432000):
                if 0.1< price <1.5 and 40 < owner_balance:
